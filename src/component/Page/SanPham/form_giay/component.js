...
function Component_type(props) {
  ...
  function closeDidalogDeleteMS() {
    const newlist = [
      ...mausac.slice(0, indexTamMS),
      ...mausac.slice(indexTamMS + 1),
    ];
    setMauSac(newlist);
    let d = deleteMS;
  ...
  function submit() {
    let ktCS = ListGiay.filter(
      (item) => item.ten_giay !== giayEditting.ten_giay
    );
  ...
  useEffect(() => {
    ...
    for (var i = 0; i < mausacEditting.length; i++) {
      let ms = ListMauSac.filter(
        (item) => item.id === mausacEditting[i].id_mau_sac
      );
      let st = [];
      for (var j = 0; j < mausacEditting[i].size.length; j++) {
        const s = ListSize.filter(
          (item) => item.id === mausacEditting[i].size[j].id_size
        );
        st.push({
  ...
  function handleCheckboxChanges(e) {
    ...
    if (e.target.checked) {
      newCheckedValues.push({
        ...
        ten_mau_sac: getMauSac(ListMauSac, mausac)[e.target.dataset.id]
          .ten_mau_sac,
        hinh_anh: "",
        size: [],
      });
  ...
  function handleCheckboxChangeSize(e) {
    ...
    let newCheckedValuesSize = sizeTam.filter(
      (item) => item.id_size !== size.id
    );
    if (e.target.checked) {
  ...
  function handleCheckboxChangeSizes(e) {
    ...
    const newlist = [
      ...tam.slice(0, e.target.dataset.id),
      datag,
      ...tam.slice(e.target.dataset.id + 1),
    ];
  ...
  function handleCheckboxChangeSizeAddMS(e) {
    ...
    const newlist = [
      ...addMS.slice(0, e.target.dataset.id),
      datag,
      ...addMS.slice(e.target.dataset.id + 1),
    ];
    setAddMS(newlist);
  }
      ...
      <form className=" tm-edit-product-form" onSubmit={handleSubmit}>
        <div className="col-xl-12 col-lg-12 col-md-12">
          ...
          <div className="form-group">
            ...
            {errors.id_loai_giay && (
              <p className="error">{errors.id_loai_giay}</p>
            )}
          </div>
            ...
            {!giayEditting.id ? (
              <div className="padding-null col-xl-12 col-lg-12 col-md-12 mx-auto mb-4">
                <div className="form-group mb-3">
                  ...
                  {errorsMS.mau_sac && (
                    <p className="error">{errorsMS.mau_sac}</p>
                  )}
                </div>
              </div>
            ) : (
                ...
                {mausac.length > 0 ? (
                  mausac.map((l, index) => {
                    return (
                      <div
                        key={index}
                        className="border-mausac col-xl-12 col-lg-12 col-md-12"
                      >
                                          ...
                                          >
                                            ...
                                            <div className="form-group mb-3 size-soluong">
                                              ...
                                              <input
                                                ...
                                                name="so_luong"
                                                type="number"
                                                data-id={addSizes(index)}
                                                data-idsizes={indexss}
                                                value={ll.so_luong}
                                                className="form-control validate"
                                                onChange={(e) =>
                                                  onChangeInputSizes(e)
                                                }
                                              />
                                            </div>
                                          </div>
