...
function SanPham(props) {
  ...
  useEffect(() => {
    ...
    if (searchSP !== "" && tamS === true) {
      delayDebounceFn = setTimeout(() => {
          ...
          .then((response) => {
            if (response.status === 200) {
              if (response.data.data.length > 0) {
                setDataList(response.data.data);
                history.push(`/SanPham/page=${1}&&search=${searchSP}`);
                setTamS(false);
              } else {
                setDataList([]);
                history.push(`/SanPham/page=${0}&&search=${searchSP}`);
                setTamS(false);
              }
            }
          })
        ...
        const dataNew = ListGiay.filter((item) =>
          item.ten_giay.toLowerCase().includes(searchSP.toLowerCase())
        );
        setAllPage(dataNew.length);
      }, 1000);
    }
        ...
        .then((response) => {
          if (response.status === 200) {
            if (response.data.success === 500) {
              setShow(false);
              setNd(
                `Bạn Không thể xóa giày này hãy cập nhật lại trạng thái của giày`
              );
              setShow(true);
              setDataTam({});
            } else {
  ...
  function deleteGiays(data) {
    setShow(true);
    setNd(
      `Bạn có chắc chăc muốn xóa sản phẩm ${data.ten_giay} ra khỏi danh sách`
    );
    setDataTam(data);
    setGiayEdittings(data.id);
  }
  function handlePageChange(pageNumber) {
    ...
    if (searchSP !== "") {
      history.push(`/SanPham/page=${pageNumber}&&search=${searchSP}`);
      ...
      history.push(
        `/SanPham/page=${pageNumber}&&ThuongHieu=${dataPage.id_loai_giay}`
      );
      setDataPage((dataPage) => ({
        ...dataPage,
        offset: pageN,
      ...
      if (parseInt(dataPage.id_loai_giay) === 0) {
        ...
        const dataNew = ListGiay.filter(
          (item) =>
            parseInt(item.id_loai_giay) === parseInt(dataPage.id_loai_giay)
        );
        setAllPage(dataNew.length);
      }
    }
  ...
  function onchangeSelect(e) {
    e.persist();
    history.push(`/SanPham/page=${1}&&ThuongHieu=${e.target.value}`);
    setActivePage(1);
    let pageN = 0;
    pageN = 0;
    setDataPage((dataPage) => ({
    ...
    if (parseInt(e.target.value) === 0) {
      ...
      const dataNew = ListGiay.filter(
        (item) => parseInt(item.id_loai_giay) === parseInt(e.target.value)
      );
      setAllPage(dataNew.length);
    }
  }
